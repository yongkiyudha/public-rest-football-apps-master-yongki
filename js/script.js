const ApiKey = "1660b33dbfa246a18bab9c39925e3bf1";
...
const informationTeam = `${baseUrl}teams/`;
...
function getListTeams() {
    ...
    fetch(teamEndPoin, fetchHeader)
        ...
        .then(resJson => {
            ...
            const info = document.querySelectorAll('.secondary-content');
            info.forEach(btn =>{
                btn.onclick = (event) => {
                  getListInformations(informationTeam + event.target.dataset.id);
                }
            })
        }).catch(err => {
            console.error(err);
...
function getListInformations(id) {
    title.innerHTML = "Informasi Tentang Tim Liga Inggris "
    fetch(id, fetchHeader)
    .then(response => response.json())
    .then(info=> {contents.innerHTML = ` 
        
        <center>
        <div class="card">
        <h2><b>${info.name}</b></h2>
        <img class="activator" src="${info.crestUrl}">
        <div class="card-stacked">
        <div class="card-content">
        <table>
        <tr>
        <td><b>Nama Pendek </b></td><td>${info.shortName}</td></tr>
        <tr>
        <td><b>Singkatan Klub </b></td><td>${info.tla}</td></tr>
        <tr>
        <td><b>Alamat </b></td><td>${info.address}<br></tr>
        <tr>
        <td><b>Nomor Handphone </b></td><td>${info.phone}<br></tr>
        <tr>
        <td><b>Website Klub</b></td><td>${info.website}<br></tr>
        <tr>
        <td><b>Email Klub</b></td><td>${info.email}<br></tr>
        <tr>
        <td><b>Tahun didirikan </b></td><td>${info.founded}<br></tr>
        <tr>
        <td><b>Warna Kebanggaan </b></td><td>${info.clubColors}<br></tr>
        <tr>
        <td><b>Markas </b></td><td>${info.venue}</br></td></tr>
        <tr>
        <td><b>Terakhir di update </b></td><td>${info.lastUpdated}</br></td></tr>
        </table>
        </div> 
    </div>
  </div>
